function [BNet, outstats] = FullBNLearn(data, cols, pheno, BFTHRESH, ...
    outfilename, priorPrecision, disc, verbose, searchParameter, targetCols, colsTypes)
...
%   Defaults to all.
% COLSTYPES: optional. specifies columns attribute types. 
%   Defaults to same type.
%
% OUTPUT:
...
ccols2learn(ctemp) = 1;
if (~isempty(colsTypes))
    dcolstype = string(zeros(1, length(dcols)));
    dcolstype = colsTypes(find(disc));
    ccolstype = string(zeros(1, length(ccols)));
    ccolstype = colsTypes(find(~disc));
end
...
if (isfield(searchParameter, 'DBN') && searchParameter.DBN)
    ...
    searchParameter.ccolstype = ccolstype;
    searchParameter.dcolstype = dcolstype;
    ...
    if (isfield(searchParameter, 'unwrapped') && searchParameter.unwrapped)
        ...
        searchParameter.ccolstype = [ccolstype, ccolstype];
        searchParameter.dcolstype = [dcolstype, dcolstype];
