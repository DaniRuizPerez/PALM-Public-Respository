...
if (~isfield(searchParameter, 'heterogeneous'))
    heterogeneous = false;
else
    heterogeneous = searchParameter.heterogeneous;
end

% Set up the network structure search
search = StateTrackerSearch(contData, discData, priorPrecision, phencol, backtracking, bf_thresh, nophenotype, true, searchParameter.ccolstype, searchParameter.dcolstype);
if (DBN)
    if (unwrapped)
        search = UnwrappedTSDBNSTS(contData, discData, priorPrecision, phencol, backtracking, bf_thresh, nophenotype, true, searchParameter.ccolstype, searchParameter.dcolstype);
        search = search.Init();
        search = search.SetUnwrappedTimeSeries();
        if (heterogeneous)
            search = search.SetInteractionMatrix(searchParameter.interactionTypes, searchParameter.interactionMatrix);
        end
        ...
        search = TSDBNStateTrackerSearch(contData, discData, priorPrecision, phencol, backtracking, bf_thresh, nophenotype, true, searchParameter.ccolstype, searchParameter.dcolstype);
        search = search.Init();
...
[search, currentNumParameters] = search.GetNumParameters(0, 0, false);
