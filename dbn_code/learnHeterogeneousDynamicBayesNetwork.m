function DBN = learnHeterogeneousDynamicBayesNetwork(filename, pheno, cols2learn, maxParents, localNetStruc, intraEdges, mle, nu, alpha, sigma2, BFTHRESH, MAXDISCVALS)
%DBN = learnHeterogeneousDynamicBayesNetwork(filename, pheno, cols2learn, maxParents, localNetStruc, intraEdges, mle, nu, alpha, sigma2, BFTHRESH, MAXDISCVALS)
%
% Constructs a Dynamic Bayesian Network (DBN).
% Outputs to both trivial graph format and SIF format for Cytoscape.
%
% INPUT:
%     filename      % filename with heterogeneous data types for learning a DBN
...
%
% Sample calls:
% G = learnHeterogeneousDynamicBayesNetwork('infant_gut_microbiota_dbn_sample_noalignment_sr3d', '', [], 5, false, false, true); %MLE 
% G = learnHeterogeneousDynamicBayesNetwork('human_vaginal_microbiota_dbn_sample_noalignment_nosplines', '', [], 5, false, false, false, 1, 1, 1); %MAP
%
    if (nargin < 12)
        MAXDISCVALS = 4;
    end
